...
export async function startServer() {
  ...
  app.use((req, res, next) => {
    ...
    res.on("finish", () => {
      if (req.url.startsWith("/api")) {
        console.log(
          `[API_TRACE] ${req.method} ${req.url} -> ${res.statusCode} (${
            Date.now() - start
          }ms)`
        );
      }
  ...
  const rateLimiter = (
    ...
    const timestamps = rateLimitMap
      .get(key)
      .filter((t: number) => t > now - window);

    if (timestamps.length >= limit) {
      return res.status(429).json({
        error: "NEURAL_BUFFER_OVERFLOW",
      ...
      if (database) {
        await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("SIEM_LOG_TIMEOUT")),
              3000
            )
          ),
        ]);
  ...
  app.use(
    (
      ...
      res.status(500).json({
        ...
        message:
          err.message ||
          "A critical neural link failure occurred.",
      });
    }
  );
